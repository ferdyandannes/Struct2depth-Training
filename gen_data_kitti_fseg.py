...
HEIGHT = 128
STEPSIZE = 1
INPUT_DIR = '/media/ferdyan/NewDisk/Dataset/KITTI_Raw/'
OUTPUT_DIR = '/media/ferdyan/NewDisk/Dataset/KITTI_procesed/'
...
for d in glob.glob(INPUT_DIR + '/*/'):
    ...
    for d2 in glob.glob(d + '*/'):
        ...
        for subfolder in ['image_02_new/data']:
            ct = 1
            ...
            calib_camera = calib_raw[0] if subfolder=='image_02_new/data' else calib_raw[1]
            folder = d2 + subfolder
            ...
            for i in range(SEQ_LENGTH, len(files)+1, STEPSIZE):
                ...
                simpan = cv2.imread(OUTPUT_DIR + seqname2 + '/' + imgnum + '-fseg.png', 0)
                print("simpan = ", simpan.shape)
                cv2.imwrite(OUTPUT_DIR + seqname2 + '/' + imgnum + '-fseg.png', simpan)
